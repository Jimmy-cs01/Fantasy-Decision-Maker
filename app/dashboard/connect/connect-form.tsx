...
export function ConnectForm() {
  ...
  return <div className="max-w-2xl">
    ...
    <div className="grid gap-5">
      <Card><p className="text-sm font-bold tracking-widest text-cyan-300">SLEEPER</p><p className="mt-2 text-slate-300">Use your public Sleeper username to find this season&apos;s NFL leagues.</p><div className="mt-5 flex gap-3"><input value={username} onChange={(event) => setUsername(event.target.value)} onKeyDown={(event) => event.key === "Enter" && findSleeperLeagues()} placeholder="Sleeper username" className="min-w-0 flex-1 rounded-lg border bg-slate-950 px-3 py-2" /><Button onClick={findSleeperLeagues} disabled={loading || !username}>{loading ? "Finding…" : "Find leagues"}</Button></div></Card>
    ...
    {sleeperLeagues.length > 0 && <Card className="mt-6"><h2 className="text-xl font-bold">Your Sleeper leagues</h2><div className="mt-4 divide-y divide-slate-800">{sleeperLeagues.map((league) => <form key={league.league_id} action={importLeague} className="flex items-center gap-4 py-4"><input type="hidden" name="username" value={username} /><input type="hidden" name="leagueId" value={league.league_id} /><div className="min-w-0 flex-1"><p className="truncate font-semibold">{league.name}</p><p className="text-sm text-slate-400">{league.season} · {league.total_rosters ?? "?"} teams</p></div><Button>Import</Button></form>)}</div></Card>}
