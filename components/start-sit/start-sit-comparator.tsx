...
export function StartSitComparator({ players, rosterPositions = [], manualRoster = false }: { players: StartSitPlayer[]; rosterPositions?: string[]; manualRoster?: boolean }) {
  ...
  const { state: manualState, update: updateManualRoster } = useManualRoster();
  ...
  const visible = players.filter((player) => {
    ...
    const matchesManual = !manualRoster || !showManualOnly || manualState.myPlayerIds.includes(player.id);
    return matchesSlot && matchesManual && (!search || player.name.toLowerCase().includes(search) || player.position?.toLowerCase().includes(search) || player.nflTeam?.toLowerCase().includes(search));
  }).slice(0, 40);
  ...
  return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(19rem,.72fr)]">
    <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-3 sm:p-4">
      ...
      {manualRoster ? <div className="mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-slate-800 bg-slate-950/45 p-2">
        <UserRoundCheck size={16} className="text-cyan-300" />
        <span className="text-xs font-bold">Manual roster: {manualState.myPlayerIds.length}</span>
        <button type="button" aria-pressed={showManualOnly} onClick={() => setShowManualOnly((value) => !value)} className="ml-auto rounded-lg border border-slate-700 px-2 py-1 text-xs font-bold hover:border-cyan-400">{showManualOnly ? "Show all players" : "Use my roster"}</button>
      </div> : null}
      ...
      <div className="mt-3 max-h-[34rem] space-y-1 overflow-y-auto pr-1">{visible.map((player) => {
        ...
        const onManualRoster = manualState.myPlayerIds.includes(player.id);
        return <div key={player.id} className={`grid w-full grid-cols-[2.25rem_minmax(0,1fr)_auto_auto] items-center gap-2 rounded-xl border px-2 py-2 text-left transition ${active ? "border-cyan-400/70 bg-cyan-400/10" : "border-transparent bg-slate-950/45 hover:border-slate-700"}`}>
          <button type="button" aria-label={`${active ? "Remove" : "Select"} ${player.name} for comparison`} aria-pressed={active} onClick={() => toggle(player.id)} className="contents">
          ...
          {manualRoster ? <button type="button" aria-label={`${onManualRoster ? "Remove" : "Add"} ${player.name} ${onManualRoster ? "from" : "to"} manual roster`} onClick={() => updateManualRoster({ ...manualState, myPlayerIds: onManualRoster ? manualState.myPlayerIds.filter((id) => id !== player.id) : [...manualState.myPlayerIds, player.id] })} className="rounded-lg border border-slate-700 p-2 text-slate-300 hover:border-cyan-400 hover:text-white">{onManualRoster ? <X size={15} /> : <Plus size={15} />}</button> : null}
        </div>;
      })}</div>
    </section>
