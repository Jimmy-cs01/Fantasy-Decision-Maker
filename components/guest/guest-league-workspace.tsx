...
import { guestLeagueHref, readGuestSession, writeGuestSession } from "@/lib/guest/session";
...
export function GuestLeagueWorkspace({ leagueId }: { leagueId: string }) {
  ...
  const view = requestedView && GUEST_VIEWS.has(requestedView) ? requestedView : "overview";
  ...
  if (session === null) return <AppShell guest guestView={view}><GuestExpired /></AppShell>;
  if (!result || result.leagueId !== leagueId) return <AppShell guest guestView={view}><p className="mx-auto max-w-6xl rounded-xl border border-slate-800 p-6 text-slate-400">Loading public Sleeper league data…</p></AppShell>;
  if (result.error || !result.payload) return <AppShell guest guestView={view}><Card className="mx-auto max-w-6xl text-center"><h1 className="text-xl font-black">Guest league unavailable</h1><p className="mt-2 text-slate-400">{result.error || "The guest session could not load this league."}</p><Link href="/guest" className="mt-4 inline-block font-black text-cyan-300">Choose another league →</Link></Card></AppShell>;
  const payload = result.payload;

  return <AppShell guest guestView={view}>
...
function GuestExpired() {
  return <Card className="mx-auto max-w-xl text-center"><h1 className="text-2xl font-black">Guest session ended</h1><p className="mt-2 text-slate-400">Enter your Sleeper username again to reload public league data.</p><Link href="/guest" className="mt-4 inline-block rounded-xl bg-cyan-400 px-4 py-2 font-black text-slate-950">Continue as Guest</Link></Card>;
}
...
function GuestOverview({ payload, teamId }: { payload: GuestLeaguePayload; teamId: string | null }) {
  ...
  return <LeagueOverview
    league={{
      id: payload.league.id, name: payload.league.name, season: payload.league.season,
      seasonType: payload.league.seasonType, totalRosters: payload.league.totalRosters,
      provider: "sleeper", rosterPositions: payload.league.rosterPositions,
      scoringAvailable: Object.keys(payload.league.scoringSettings).length > 0,
      ephemeral: true,
    }}
    teams={payload.teams.map((team) => ({
      id: team.id, displayName: team.name, wins: team.wins, losses: team.losses,
      ties: team.ties, isMyTeam: team.isMyTeam, players: team.players, summary: team.summary,
    }))}
    selectedTeamId={teamId}
    teamHref={(id) => `${guestLeagueHref(payload.league.id)}?teamId=${encodeURIComponent(id)}`}
...
function GuestTrades({ payload }: { payload: GuestLeaguePayload }) {
  ...
  return <div className="mx-auto max-w-7xl"><h1 className="text-3xl font-black">Trade Finder</h1><p className="mt-2 text-sm text-slate-400">Uses {payload.league.name}&apos;s scoring, roster demand, and public Sleeper ownership.</p><div className="mt-5"><TradeFinder teams={teams} rosterPositions={payload.league.rosterPositions} analyticsAvailable={payload.analyticsAvailable} leagueTeams={payload.league.totalRosters} /></div></div>;
}
...
function GuestStartSit({ payload }: { payload: GuestLeaguePayload }) {
  ...
  return <div className="mx-auto max-w-7xl"><h1 className="text-3xl font-black">Start / Sit</h1><p className="mt-2 text-sm text-slate-400">Compare your roster using actual league scoring and legal lineup slots.</p><div className="mt-5"><StartSitComparator players={players} rosterPositions={payload.league.rosterPositions} /></div></div>;
}
