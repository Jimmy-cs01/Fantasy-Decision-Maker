...
describe("authentication validation", () => {
  it("validates login credentials", () => {
    expect(loginSchema.safeParse({ email: "fan@example.com", password: "secret1" }).success).toBe(true);
    expect(loginSchema.safeParse({ email: "not-email", password: "short" }).success).toBe(false);
  });

  it("requires matching signup passwords", () => {
    expect(signupSchema.safeParse({ email: "fan@example.com", password: "secret1", confirmPassword: "secret1" }).success).toBe(true);
    expect(signupSchema.safeParse({ email: "fan@example.com", password: "secret1", confirmPassword: "different" }).success).toBe(false);
  });
