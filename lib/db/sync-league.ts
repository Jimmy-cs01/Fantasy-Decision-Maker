...
export async function synchronizeLeague(db: Database, ownerId: string, sleeperUser: SleeperUser, sleeperLeagueId: string) {
  ...
  const { data: localLeague, error: leagueError } = await db.from("leagues").upsert({ ...normalizeLeague(league), owner_id: ownerId, last_synced_at: new Date().toISOString() }, { onConflict: "owner_id,sleeper_league_id" }).select().single(); ensure(leagueError);
  ...
  const members = users.map((user) => ({ league_id: localLeague.id, sleeper_user_id: user.user_id, username: user.username, display_name: user.display_name ?? null, avatar_id: user.avatar ?? null }));
  ...
  const teams = sleeperRosters.map((r) => { const n = normalizeRoster(r); return { league_id: localLeague.id, league_member_id: r.owner_id ? memberBySleeperId.get(r.owner_id) ?? null : null, sleeper_roster_id: n.sleeper_roster_id, name: n.name, wins: n.wins, losses: n.losses, ties: n.ties }; });
  ...
  for (const remoteRoster of sleeperRosters) { const team = localTeams.find((t: any) => t.sleeper_roster_id === remoteRoster.roster_id); if (!team) continue; const n = normalizeRoster(remoteRoster); const { data: roster, error } = await db.from("rosters").upsert({ fantasy_team_id: team.id, starters: n.starters, reserve: n.reserve }, { onConflict: "fantasy_team_id" }).select().single(); ensure(error); ensure((await db.from("roster_players").delete().eq("roster_id", roster.id)).error); const remotePlayers = n.players.map((id) => allPlayers[id]).filter(Boolean).map(normalizeSleeperPlayer); if (remotePlayers.length) { ensure((await db.from("players").upsert(remotePlayers, { onConflict: "sleeper_player_id" })).error); const { data: localPlayers, error: playersError } = await db.from("players").select("id,sleeper_player_id").in("sleeper_player_id", remotePlayers.map((p) => p.sleeper_player_id)); ensure(playersError); ensure((await db.from("roster_players").insert(localPlayers.map((p: any) => ({ roster_id: roster.id, player_id: p.id, is_starter: n.starters.includes(p.sleeper_player_id) })))).error); } }
